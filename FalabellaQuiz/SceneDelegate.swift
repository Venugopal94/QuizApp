...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let userInfo = RealmManager.shared.fetchObjects(User.self), !userInfo.isEmpty {
            navigateToQuizVc()
        } else {
    ...
    func navigateToRegisterVC() {
        guard let registrationVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: UserRegistrationVC.className) as? UserRegistrationVC else { return }
