...
class NetworkHandler: NSObject {
    ...
    private final let newProdUrl = "https://api.snglty.com/v1"
    ...
    func dataRequest<T: Decodable>(method: MethodType,with url: ApiEndPoints, body: [String : AnyObject]?, params: String, objectType: T.Type, completion: @escaping (_ responseModel: T?, _ error: String?) -> Void) {
        
        guard reachability?.isReachable ?? false else {
            completion(nil, NetWorkErrors.noInternent.rawValue)
            return
        }

        let session = URLSession.shared
        let requestURL = newProdUrl + url.rawValue + params
